...
for pkg in ("sympy_editor_tree", "sympy_editor_plot", "sympy_editor_matching", "sympy_editor_feynman"):
...
def addons():
    ...
    return [tree, plot, matching, feynman]
...
def main(argv=None) -> int:
    ...
    x = symbols("x")
    tree, plot, matching, feynman = addons()
    # Two on to start with, the others a click away in the Add-ons menu.
    doc = Document(sin(x) ** 2 / x + cos(x) ** 2, addons=[tree, plot], available=[matching, feynman])
    if args.serve:
